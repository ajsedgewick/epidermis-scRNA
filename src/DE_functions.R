...
clustDE <- function(coldata, countdata, outfile="clusterDE.xlsx", logcpm=F){
...
clustDE2 <- function(coldata, countdata, outfile="clusterDE.xlsx"){
  #coldata$cluster <- as.factor(cluster)
  coldata$cluster <- droplevels(coldata$cluster)
  cur.k <- length(levels(coldata$cluster))
  
  design <- model.matrix(~0 + cluster + percent.mito, data=coldata)
