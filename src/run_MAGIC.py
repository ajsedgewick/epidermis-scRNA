...
sys.path.insert(0 , "/magic/src/")
from  magic.MAGIC_core import impute_fast ,  compute_markov
import time

import feather_utils #need to make sure this is on the path
...
def magic_progressive_t(data_df, dist_data_df , t_list, 
    ...
    for t in t_list:
        ...
        if fo_imputed:
            ...
            #                                                       float_format = "%.4f")
            feather_utils.export_feather(pd.DataFrame( data_new , index = data_df.index.values ,
                                                       columns = data_df.columns.values  ), fo_imputed.format(t))
...
if __name__=="__main__":
    ...
    parser.add_argument('--fi_distData' , help = "csv file of cell coordinates used to compute cell cell distance.   Rows index cells,  cols index coordinates (e.g PC cocrinates for entries of zinb-wave W matrix for the samme cell)." , required =True )
    parser.add_argument('--fo_imputed' , help="eg. baseName_t{}.csv. {} filled in with appropriate t" , required=True)
    parser.add_argument('--fo_details' , help= 'file for output of details on run parameters and timing)' )
    ...
    parser.add_argument('--libNormalize' , action = 'store_true' , help ="Perform magic library size normalization on fi_expr before imputation")
    ## Magic Parameters
    parser.add_argument('--tList', help='comma separated list of t values. Results for each of these t values are saved', required = True)
    parser.add_argument('-k' , type = int , default = 30, help = "MAGIC parameter")
    parser.add_argument('--ka' , type = int , default = 10, help = "MAGIC parameter")
    parser.add_argument( '--epsilon', type = int , default = 1, help = "MAGIC parameter")
    parser.add_argument('--rescale' , type =int , default = 99, help = "MAGIC parameter")
    ...
    data_df = feather_utils.import_feather(args.fi_expr)
    if args.geneRows:
