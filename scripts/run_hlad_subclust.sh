...
# Specify cells use exclusively for PCA/subclustering 
...
mkdir -p inputs
awk -F "," 'NR==1 {for(i=1;i<=NF;i++){if ($i ~ /cluster/) {clcol=i}}}
            NR>1 && $clcol==11 {print $1}' ./cl_immune/coldata_clust.csv > inputs/${PREFIX}_cell_names.txt
